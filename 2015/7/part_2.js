...
for (const character of input + " ") {
    index++
    if (character == ">") counter1++
    if (counter1 == 1) {
        if (character == " ") counter2++
        if (counter2 == 2) {
            instructions.push(input.slice(lastIndex, index))
            lastIndex = index + 1
            counter1 = 0
            counter2 = 0
        }
...
let value1, value2, operator, answer
let solvedVariables = {}

function constructCircuit(kitInstructions) {
    loop: for (const instruction of kitInstructions) {
        ...
        if (split.length == 5) {
            ...
            else if (solvedVariables[value2] != undefined) value2 = solvedVariables[value2]
            else continue loop

            if (operator == "AND") solvedVariables[answer] = value1&value2
            if (operator == "OR") solvedVariables[answer] = value1|value2
            if (operator == "LSHIFT") solvedVariables[answer] = value1<<value2
            if (operator == "RSHIFT") solvedVariables[answer] = value1>>value2

            kitInstructions.splice(kitInstructions.indexOf(instruction), 1)
            constructCircuit(kitInstructions)
        }
...
constructCircuit(instructions)
console.log(solvedVariables.a)
