...
for (let variation of allPossibleOutcomes(...uniqueNames)) {
    for (let i = 1; i < variation.length; i++) {
        if (variation[i] == "Santa" || variation[i - 1] == "Santa") continue

        happiness += happinessValues[`${variation[i - 1] + variation[i]}`]
        happiness += happinessValues[`${variation[i] + variation[i - 1]}`]
    }

    if (variation[0] != "Santa" && variation[8] != "Santa") {
        happiness += happinessValues[`${variation[0] + variation[8]}`]
        happiness += happinessValues[`${variation[8] + variation[0]}`]
    }
